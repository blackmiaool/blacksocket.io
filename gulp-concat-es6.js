...
function map({ name: file }, opt) {
    ...
    function bufferContents(file, enc, cb) {
        ...
        if (file.isStream()) {
            this.emit('error', new Error('gulp-concat-es6: Streaming not supported'));
            cb();
            return;
        }
        ...
        let content = String(file.contents).split("\n").filter((line) => {
            if (line.match(/=\s*require\s*\(/))
                return false;

            if (line.match(/module.exports\s*=/))
                return false;
            if (line.match(/\bexport /))
                return false;
            return true;
        }).join('\n');

        concat.add(file.relative, new Buffer(content), file.sourceMap);
        cb();
    }
